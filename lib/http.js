...
const commonRequest = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    request[method](`${host || config.DEVOPS_PROD_HOST}${url}`, params, (err, res, data) => {
      if (err) {
        return reject(err)
      }
...
const sendDingTalk = ({ url, at = [], apply_info = {} }) => {
  ...
  // const configAt = config.successDing ? config.successDing.at : []
  ...
  request.post(dingtalkUrl, {
    ...
    body: {
      ...
      markdown: {
        title: '来活了老板',
        text: `## devops-prod${apply_app_name}${apply_app_type}${apply_tag}${apply_name}${apply_publish_desc}${apply_status_desc}`
      },
      at: {
        atMobiles: ['18084621629'],
        isAtAll: true
      }
    }
  })
...
const sendNewDingTalk = ({ url, at = [], apply_info = {} }) => {
  ...
  request.post(dingtalkUrl, {
    ...
    body: {
      ...
      markdown: {
        title: '来活了老板',
        text: `## devops-new${apply_app_name}${apply_tag}${apply_name}${apply_publish_desc}${apply_status_desc}`
      },
      at: {
        atMobiles: ['18084621629'],
        isAtAll: true
 ...
 * @Date: 2020-10-11 16:45:51
 */
const devposNewCreateCDNTask = (formData) => commonRequest({
  reqVersion: config.VERSION_2,
  host: config.DEVOPS_NEW_HOST,
  url: 'gateway/add_publish_task',
  formData
