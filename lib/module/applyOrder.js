...
const chooseVersionScope = async (ftid = '') => {
  const allowUploadCDN = getData('appInfo').app_type === 10
  inquirerHandle([
    {
      name: 'auto_tag',
      message: '请选择是否需要打tag',
    ...
    },
    {
      ...
      pageSize: config.COMMAND_LINE,
      when: () => allowUploadCDN && !(config.ORDER && config.ORDER.push_cdn_type)
    },
    {
      name: 'cdn_upload_dir',
      message: '请输入上传CDN目录',
      type: 'input',
      validate: (res) => !!res,
      when: ({ push_cdn_type }) => allowUploadCDN && [2, 3].includes(push_cdn_type) && !(config.ORDER && config.ORDER.cdn_upload_dir)
    },
    {
    ...
    Object.assign(publishInfo, {
      ...
      push_cdn_type: isEmpty(push_cdn_type) ? 1 : Number(push_cdn_type),
      cdn_upload_dir: isEmpty(cdn_upload_dir) ? '' : Number(cdn_upload_dir),
