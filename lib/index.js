...
const { inquirerHandle, compressFile, getNextTag, setNewTagAndPush } = require('./utils');
...
const chooseVersionScope = async (ftid = '') => {
  inquirerHandle([
    ...
    setGlobalData('otherInfo', { ...store.otherInfo, vid, desc })
    getTagInfo(vid)
  })
}
...
const getTagInfo = async (vid = 2) => {
  try {
    ...
    nextTag && setTagForCommit(nextTag)
  } catch (error) {
    spinner.stop()
...
const setTagForCommit = async (tag) => {
  const { desc } = store.otherInfo
  const spinner = ora('正在写入最新的tag并push, 请稍后...');
  spinner.start()
  setNewTagAndPush(tag, desc, () => {
    spinner.stop()
    console.log('tag更新且push成功！')
  }, (err) => {
    spinner.stop()
...
const run = () => {
  inquirerHandle([{
    ...
    name: 'pass', // answer的key
    message: '代码是否合了最新代码？', // question标题
    type: 'list',
